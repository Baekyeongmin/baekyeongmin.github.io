...
각각의 턴에서 DM은 `input frame`으로 부터 전달받은 유저 의도와 토픽에 대한 조건을 만족시키는 대화 전략을 확인합니다. `master processing`에서는 조건을 만족시키는 미니스킬을 확인합니다. 먼저, state를 이용할지(의존할지)를 판단하고, 이를 통해 `state-independant`, `state-dependant` 대화 전략이 구분되어 실행됩니다. 대화 시그먼트가 명확히 초기화되는 경우(명확한 토픽 요청, 다른 명령의 타입 등)에 `state-independant` 전략을 이용합니다. 이 두 경우 모두 사용자의 의도/토픽에 대한 제약조건을 만족시키는 미니스킬들을 투표합니다. 가장 자세한 토픽에 대응되는 미니스킬이 우선권을 갖지만, 나머지의 경우 연속된 턴에서 같은 미니스킬의 사용을 피하기 위해 제약조건을 만족하는 미니스킬들 중에서 랜덤하게 선택합니다. 여러 미니스킬은 서로 다른 대화 전략을 갖고 다른 시스템 액션을 취하며, 특정 대화모드의 시그맨트를 관리합니다. 최종적으로 DM모듈은 NLG모듈에서 이용될 speech act를 만들고, dialogue state를 업데이트 합니다. DM 모듈의 기능을 크게 나눠보면 다음과 같습니다.

**Content retrieval**: DM모듈에 포함된 특정 미니스킬에서는 컨텐츠 검색에 대한 명령, 새로운 컨텐츠를 제시하여 대화를 이끌어 나가고 싶은 경우 등 컨텐츠를 찾아야 하는 엑션을 포함합니다. 컨텐츠 검색 시에는 NLU에서 결과로 만들어진 `input frame`을 만족하는 컨텐츠를 찾으려고 시도합니다. 이 때 실패를 줄이기 위해, 검색의 제약 조건을 점점 완화시켜나가는 backoff 전략이 사용됩니다. 먼저 `primaryIntent`(컨텐츠의 타입), `primaryTopic`을 모두 만족하는 컨텐츠를 찾고, 이를 만족하지 못하는 경우 `primaryIntent`, `primaryTopic` 순으로 조건을 완화시킵니다. 컨텐츠 타입의 경우 조건을 완전히 제거하고, 토픽 조건은 `candidateTopic`을 이용합니다. 완화를 진행했음에도 불구하고 컨텐츠가 검색되지 않는 경우, 사용자에게 이 토픽에 대해 말할 것이 없음을 알리는 실패 액션이 실행됩니다.
...
**Satisfaction detection**: 시스템이 불쾌한, 모욕적인 컨텐츠 혹은 지루한 컨텐츠를 제공했을 때, 문제가 발생할 수 있습니다. 이러한 경우 유저는 명확하게 토픽 변화를 요청할 수 있지만, 불만을 표시하는 경우가 많았습니다. 이 때 시스템은 문제를 발견하고 토픽 변화를 초기화 해야합니다. 저자들은 이 문제를 발견하기 위한 이진 분류기를 만들고자 했습니다. 사용자 만족은 토픽의 변화와 관련이 있다고 판단하여, 토픽이 바뀌어야 하는지를 판단하는 문제를 풀었습니다. (단순히 만족도를 판단하는 것 보다 이 경우가 더 일관성있는 레이블을 받을 수 있었기 때문에) 2381개의 데이터를 만들었고, 적은 양의 데이터로 인해 n-gram(1,2,3 gram) 피쳐를 이용한 logistic regression classifier를 만들었습니다.

**Speech act selection**: DM은 유저의 대화(턴)을 다루기 위해, 미니스킬에 따라 여러 액션을 실행합니다. 여기서 미니스킬은 결과로 사전에 정의된 speech act 중 1개 이상을 선택하도록 요구됩니다. speech act는 크게 `grounding`, `inform`, `request`, `instruction`의 대분류로 구분됩니다.
- `grounding`: [grounding](https://en.wikipedia.org/wiki/Grounding_in_communication)은 유저와 대화를 충분히 이해하고 있음을 알려주기 위한 speech act입니다. [`back-channel`](https://en.wikipedia.org/wiki/Backchannel_(linguistics)), `echo of user request for confirmation`, 3가지 문제 인정, 감사의 6개로 새분화 됩니다. 이러한 피드백은 유저의 발화를 잘 받았고, 이해하고 있다는 것을 알릴 수 있으므로 중요한 역할을 합니다. `input frame`과 컨텐츠 검색 결과, 토픽 변화 감지, 에러 헨들링 등에 의해 결정됩니다.
- `inform`: 유저에게 컨텐츠가 제공되어야 하는 경우에 제대로된 컨텐츠가 찾아져서 쌍이 이뤄진 경우 사용됩니다.
- `request`: 확인 질문, 토픽에 대한 요청, 사용자가 의견을 제시하도록 제안하는 것 등이 포합됩니다.
  - `inform`는 `request`와 주로 결합되고, 이를 통해 시스템이 제공한 정보(inform)에 대한 유저의 반응 및 커멘트(request)를 받도록 유도합니다. 하지만 사용자에게 자극을 줄 수 있는 확인과정은 최소화합니다.
...
Sounding Board는 사용자들을 특정 타입중 하나로 매핑하여 유지하였습니다. 몇 개의 짧은 질문을 통해 사용자를 4가지 성격 사분면 중 하나에 위치시켰는데, 4분면의 두축은 외향성과 솔직함으로 외향성은 얼마나 수다스럽고 사교적인지를 나타내고, 수용성은 지적, 예술적으로 호기심을 나타내는 것과 관련이 있습니다.

5개의 질문을 진행한 후에 그들의 성격 결과를 얻을 수 있는 옵션을 제공합니다. 이 때, 중간중간에 대화를 지속하기 위한 "goofy" 질문을 끼워넣는 전략을 이용했습니다. 각 질문에 대한 대답을 얻었을 때, 다음 질문을 하기 전에 반응또한 제공했습니다. (유저 피드백에서 이 반응이 유저 경험에서 큰 향상을 가져옴.) 각 성격 질문은 각 차원에 긍정적/부정적으로 로드되며 이를 통해 사용자를 사분면 상에 위치 시킵니다. 그 후에 사용자를 디즈니 캐릭터에 할당하고 이를 알려줍니다. 성격 질문에 답한 사용자에게는 각 사분면의 유형이 관심을 가질만한 것에 기초하여 토픽을 갱신합니다.
